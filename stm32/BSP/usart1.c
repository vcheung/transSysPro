...
void USART1_Config(void)
{
	...
	/* 配置USART1的工作模式：波特率115200,8个数据位，1个停止位，无硬件流控制 */
 ...
 *
 * 为了实现重定向 printf()函数，我们需要重写 fputc() 这个c标准库函数，
 * 因为 printf()在c标准库函数中实质是一个宏，最终是调用了 fputc()这个函数的。
 *
 * 函数名：fputc
 * 描述  ：重定向c库函数printf到USART1
 * 输入  ：无
 * 输出  ：无
 * 调用  ：由printf调用（在main.c中）
 */
int fputc(int ch, FILE *f)	// FILE的声明在<stdio.h>
{
	/* 将Printf内容发往串口 */
